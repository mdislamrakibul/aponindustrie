                        ...
                        <div class="row product-grid-3">
                            ...
                            @foreach ($products as $prod)
                                    ...
                                    'product_id' => $prod['id']
                                                                        ...
                                                                        @php $mediaItems = collect($prod['media']); @endphp

                                                                        @if($mediaItems->count() > 0)
                                                                                                            <img class="default-img"
                                                                                                                src="{{ asset($mediaItems->first()['file_path'] . $mediaItems->first()['image_name']) }}"
                                                                                                                alt="{{ $prod['name'] }}">
                                                                                                            <img class="hover-img"
                                                                                                                src="{{ asset($mediaItems->count() > 1
                                                                            ? $mediaItems->get(1)['file_path'] . $mediaItems->get(1)['image_name']
                                                                            : $mediaItems->first()['file_path'] . $mediaItems->first()['image_name']) }}"
                                                                                                                alt="{{ $prod['name'] }}">
                                                                        @else
                                                                            <img class="default-img" src="{{ asset('admin/no-image.png') }}"
                                                                                alt="{{ $prod['name'] }}">
                                                                            <img class="hover-img" src="{{ asset('admin/no-image.png') }}"
                                                                                alt="{{ $prod['name'] }}">
                                                                        @endif
                                                                ...
                                                                <div class="product-badges product-badges-position product-badges-mrg">
                                                                    ...
                                                                    @if(!empty($prod['is_discounted']))
                                                                        <span class="badge sale">{{ number_format($prod['total_discount_percent'] ?? 0, 0) }}% Off</span>
                                                                    @endif
                                                                </div>
                                                            </div>
                            ...
                            @foreach ($newProduct as $item)
                                    ...
                                    $item['name'] }}</a></h5>
                                                            ...
                                                            <div style="display: flex; gap: 8px;">
                                                                ...
                                                                <div>
                                                                    @for ($i = 1; $i <= 5; $i++) @if ($i <= $rating) <i class="fa fa-star"
                                                                            style="color: #f15412;"></i>
                                                                        @elseif (($i - 0.5) <= $rating) <i class="fa fa-star-half-empty"
                                                                            style="color: #f15412;"></i>
                                                                        @else
